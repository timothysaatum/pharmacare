...
export const useAuthStore = create<AuthState>((set) => ({
    ...
    initialize: async () => {
        ...
        try {
            ...
            if (token && user) {
                ...
                // Resume sync engine if we have an active branch (e.g. app restart)
                if (branchId) {
    ...
    login: async (username, password) => {
        ...
        set({ user: data.user, isAuthenticated: true, activeBranchId: branchId });

        // Start sync engine as soon as we have a branch
        if (branchId) {
            syncEngine.start(branchId);
        }
    },
    ...
    logout: async () => {
        // Stop sync before clearing tokens so any final push can still auth
        syncEngine.stop();
    ...
    setActiveBranch: (branchId) => {
        ...
        // Switch the sync engine to the newly selected branch
        syncEngine.stop();
        syncEngine.start(branchId);
    },
}));